    ...
    return (
        <div className="flex flex-wrap landscape:flex-col justify-center items-center gap-4 p-6 pb-[calc(env(safe-area-inset-bottom)+1.5rem)] landscape:p-4 landscape:pb-4 bg-black/80 backdrop-blur-xl rounded-t-3xl landscape:rounded-t-none landscape:rounded-l-3xl border-t landscape:border-t-0 landscape:border-l border-white/10 w-full landscape:w-auto fixed bottom-0 left-0 landscape:left-auto landscape:right-0 landscape:top-0 landscape:bottom-0 z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] h-auto landscape:h-full overflow-y-auto">
            ...
            <div className="flex landscape:flex-col gap-3">
                <button onClick={() => onModeChange('INTRO')} className="flex items-center gap-2 px-5 py-3 min-h-[44px] bg-green-600/20 hover:bg-green-600 text-green-400 hover:text-white border border-green-600/50 font-bold rounded-xl transition-all backdrop-blur-sm landscape:justify-center">
                    <Music size={20} /> <span className="hidden md:inline landscape:hidden xl:inline">Intro Quiz</span>
                </button>

                <button onClick={() => onModeChange('TEACHER')} className="flex items-center gap-2 px-5 py-3 min-h-[44px] bg-blue-600/20 hover:bg-blue-600 text-blue-400 hover:text-white border border-blue-600/50 font-bold rounded-xl transition-all backdrop-blur-sm landscape:justify-center">
                    <User size={20} /> <span className="hidden md:inline landscape:hidden xl:inline">Teacher</span>
                </button>

                <button onClick={onAddReach} className="hidden md:flex items-center gap-2 px-5 py-3 min-h-[44px] bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white border border-red-600/50 font-bold rounded-xl transition-all backdrop-blur-sm landscape:justify-center">
                    <Plus size={20} /> <span className="hidden md:inline landscape:hidden xl:inline">Add Reach</span>
                </button>

                <button onClick={() => onModeChange('SUCCESS')} className="flex items-center gap-2 px-5 py-3 min-h-[44px] bg-purple-600/20 hover:bg-purple-600 text-purple-400 hover:text-white border border-purple-600/50 font-bold rounded-xl transition-all backdrop-blur-sm landscape:justify-center">
                    <Trophy size={20} /> <span className="hidden md:inline landscape:hidden xl:inline">Bingo!</span>
                </button>

                <button onClick={onReset} className="flex items-center gap-2 px-5 py-3 min-h-[44px] bg-gray-600/20 hover:bg-red-600/80 text-gray-400 hover:text-white border border-gray-600/50 font-bold rounded-xl transition-all backdrop-blur-sm landscape:justify-center">
